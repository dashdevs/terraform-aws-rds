...
| <a name="input_rds_storage_autoscaling_max_gib"></a> [rds\_storage\_autoscaling\_max\_gib](#input\_rds\_storage\_autoscaling\_max\_gib) | Upper bound in GiB for RDS storage autoscaling. Set to `null` to disable autoscaling. Must be greater than or equal to `rds_allocated_storage`. | `number` |`null`| no |
